...
# tar 파일의 경로
tar_path = '/content/drive/My Drive/VOCtrainval_11-May-2012.tar'

# 압축 해제를 위한 함수
def extract_tar(tar_path, path_to_extract='/content'):
    with tarfile.open(tar_path) as tar:
        tar.extractall(path=path_to_extract)
        print(f'Extracted tar file {tar_path} to path {path_to_extract}')

# 압축 해제 실행
extract_tar(tar_path)

# 압축 해제 후 데이터셋의 경로 업데이트

#data_dir = "../VOCdevkit/VOC2012"
data_dir = "/content/VOCdevkit/VOC2012"
...
PATH = '/content/UNet_trained_model.pth'
#PATH = '/content/resnet_encoder_unet.pth'

loadmodel = torch.load(PATH, map_location=device)
