...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h = np.dot(X, W1) + b1
    hidden = np.maximum(0, h) #Relu activation
    ...
    sum_exp_scores = np.sum(exp_scores, axis=1).reshape(N, 1)
    ...
    grads['W2'] = np.dot(hidden.T, dscores)      # (H, C)
    ...
    grads['W1'] = np.dot(X.T, dhidden)        # (D, H)
